...
model = GAT(in_channels=Gs.x.shape[1], hidden_channels=512, out_channels=256).to(device)
...
loss_fn = nn.BCEWithLogitsLoss()  # 使用BCEWithLogitsLoss替代BCELoss
...
print("\n开始训练干净模型...")
# 初始化新的干净模型
clean_sage = GAT(in_channels=Gs.x.shape[1], hidden_channels=512, out_channels=256).to(device)
...
clean_loss_fn = nn.BCEWithLogitsLoss()  # 使用BCEWithLogitsLoss
...
for epoch in clean_epoch_pbar:
    ...
    for idx_Gs, idx_Gt, labels in batch_pbar:
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = clean_loss_fn(pred, labels.float())
