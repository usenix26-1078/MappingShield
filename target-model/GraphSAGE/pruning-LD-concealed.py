...
for ratio in r:
    ...
    # 定义文件夹名称
    save_path = f"pruning_results/t_{similarity_threshold}_alpha_{alpha}_beta_{beta}_p_t_{p_t}_ratio_{ratio}"
    # 创建文件夹
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    similarity_analysis(Gs_new,train_trigger_s,'s',save_path)
    similarity_analysis(Gt_new,train_trigger_t,'t',save_path)
