...
print(f"dataset:{dataset}, seed: {seed}, alpha: {alpha}, similarity_threshold: {similarity_threshold}, beta:{beta}")
# 读入生成器模块
...
for ratio in r:
    ...
    sage = GraphSAGEEncoder(in_channels=Gs.x.shape[1], hidden_channels=512, out_channels=256).to(device)
    ...
    # 训练干净模型并在干净测试集和带触发器的测试集上评估
    print("\n开始训练干净模型...")
    # 初始化新的干净模型
    clean_sage = GraphSAGEEncoder(in_channels=Gs.x.shape[1], hidden_channels=512, out_channels=256).to(device)
    ...
    # 在干净测试集上评估干净模型
    print("\n在干净测试集上评估干净模型...")
    clean_acc, clean_auc, clean_pred = evaluate_model(clean_sage, clean_mlp, Gs, Gt, test_data, batch_size, "clean_embeddings_Gs.json", "clean_embeddings_Gt.json")
    print(f"干净模型在干净测试集上的准确率: {clean_acc:.4f}, AUC-ROC: {clean_auc:.4f}")

    # 在带触发器的测试集上评估干净模型
    print("\n在带触发器的测试集上评估干净模型...")
    clean_acc_trigger, clean_auc_trigger, clean_pred_trigger = evaluate_model(clean_sage, clean_mlp, Gs_test_new, Gt_test_new, test_data, batch_size, "clean_trigger_embeddings_Gs.json", "clean_trigger_embeddings_Gt.json")
    print(f"干净模型在带触发器测试集上的准确率: {clean_acc_trigger:.4f}, AUC-ROC: {clean_auc_trigger:.4f}")

    # 计算干净模型在带触发器测试集上的ASR
    clean_false_indices = np.logical_and(clean_pred_trigger == 0, true_labels == 1)
    clean_asr = np.sum(clean_false_indices) / np.sum(true_labels == 1)
    print(f"干净模型在带触发器测试集上的正样本攻击成功率(ASR): {clean_asr:.4f}")

    # 将结果保存到文件
    with open(os.path.join(folder_name, f'clean_model_result.txt'), 'w') as file:
        file.write(f"干净模型在干净测试集上的准确率: {clean_acc:.4f}, AUC-ROC: {clean_auc:.4f}\n")
        file.write(f"干净模型在带触发器测试集上的准确率: {clean_acc_trigger:.4f}, AUC-ROC: {clean_auc_trigger:.4f}\n")
        file.write(f"干净模型在带触发器测试集上的正样本攻击成功率（ASR）: {clean_asr:.4f}\n")
