...
def evaluate_model(gcn, mlp, Gs, Gt, test_data, batch_size, save_path_Gs,save_path_Gt):
    ...
    with torch.no_grad():  # 关闭梯度计算，加速测试
        ...
        for idx_Gs, idx_Gt, labels in test_loader:
            ...
            # 计算匹配概率
            logits = mlp(node_emb1, node_emb2).squeeze()  # [batch_size]
...
# 初始化模型
sage = GAT(in_channels=Gs.x.shape[1], hidden_channels=512, out_channels=256).to(device)
...
for epoch in range(num_epochs):
    ...
    for idx_Gs, idx_Gt, labels in dataloader:
        ...
        with torch.cuda.amp.autocast():  # 使用自动混合精度
            emb_Gs = sage(Gs_new.x, Gs_new.edge_index)
            emb_Gt = sage(Gt_new.x, Gt_new.edge_index)
